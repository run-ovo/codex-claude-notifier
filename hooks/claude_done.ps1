...
try {
    if (-not [string]::IsNullOrWhiteSpace($raw)) {
        ...
        if ($payload.PSObject.Properties.Name -contains "hook_event_name" -and $payload.hook_event_name -ne "Stop") {
            $notify = $false
        }
    }
} catch {
...
$transcriptPath = Get-ClaudePayloadValue $payload @("transcript_path")
$rawPreview = Get-TruncatedText $raw 4096

Write-ClaudeHookLog ("Stop hook fired; event={0}; notify={1}; session={2}; cwd={3}; transcript={4}; stdinLength={5}; payload={6}" -f $eventName, $notify, $sessionName, $cwd, $transcriptPath, $raw.Length, $rawPreview)

if ($notify -and (Test-Path -LiteralPath $workerFile)) {
    try {
        $cmd = "& " + (Quote-PSArg $workerFile) +
            " -Kind 'done' -Title 'Claude Code' -Message 'Task completed' -Subtitle 'Claude Code turn finished.' -SessionName " +
            (Quote-PSArg $sessionName) + " -Sound 'on' -LogFileName 'claude_code.log' -ToastTag 'claude-code-wsl-toast-notifier' -SoundMutexName 'Global\ClaudeCodeNotifySoundMutex' -LastSoundFileName 'claude_last_sound.txt'"
        ...
        Write-ClaudeHookLog ("Stop worker launch failed: " + $_.Exception.Message)
    }
}
