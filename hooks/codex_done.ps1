...
try {
    if (-not [string]::IsNullOrWhiteSpace($raw)) {
        ...
        if ($payload.PSObject.Properties.Name -contains "last_assistant_message") {
            $notify = -not [string]::IsNullOrWhiteSpace([string]$payload.last_assistant_message)
        }
    }
} catch {
    $notify = $true
...
Write-CodexHookLog ("Stop hook fired; notify={0}; session={1}; stdinLength={2}" -f $notify, $sessionName, $raw.Length)

if ($notify -and (Test-Path -LiteralPath $workerFile)) {
    try {
        $cmd = "& " + (Quote-PSArg $workerFile) +
            " -Kind 'done' -Title 'Codex' -Message 'Task completed' -Subtitle 'Codex turn finished.' -SessionName " +
        ...
        Write-CodexHookLog ("Stop worker launch failed: " + $_.Exception.Message)
    }
}
